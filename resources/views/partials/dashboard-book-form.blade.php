<div class="card border-dark ml-3 mb-3">
  <div class="card-header text-white bg-primary"><strong>Nova knjiga</strong></div>
  <div class="card-body">
...
<p class="help-block"><small>naziv knjige</small></p>
</div>
...
<p class="help-block"><small>autor</small></p>
 ...
 <div class="form-group">
  ...
  <div class="card-text collapse" id="collapse1">
    <div>
      <form action="{{route('store.media.book', ['id' => $resource->id])}}" class="dropzone" id="my-awesome-dropzone">{{ csrf_field() }}</form>
 ...
 <h5 class="card-title mb-3"><a data-toggle="collapse" href="#collapse2">+ Dodaj detalje</a></h5>
  ...
  <hr class="style13">
    ...
    <form method="POST" action="{{route('store.book', ['id' => $resource->id])}}" id="create">
        {{ csrf_field() }}  
      <span class="ml-3">
