<div class="card border-light mb-3">
  <div class="card-header text-black bg-light"><strong>Antikvarnica - kategorije</strong></div>
  <div class="card-body">
...
@foreach($categories as $category)
  <?php $this_category = $category->id ?>
      <form method="POST" id="delete:{{$category->title}}" action="{{route('remove.category.item', ['id' => $this_category])}}">
        {{ csrf_field() }} 
      </form>
      ...
      <li>
       <strong>{{$category->title}}</strong>&nbsp;&nbsp;<button type="submit" form="delete:{{$category->title}}" class="btn btn-sm btn-link" onclick="confirm('ukloni?')">(delete)</button> 
        ...
        <form method="POST" id="delete:{{$subcat->title}}" action="{{route('remove.subcategory.item', ['id' => $this_subcategory])}}">
          {{ csrf_field() }} 
        </form>
